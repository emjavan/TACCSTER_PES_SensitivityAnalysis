...
state_pop_by_age = read_csv("../data/all_US_county_pop_by_age_2019-2023ACS.csv") %>%
  mutate(
...
# Ped VC
ped_flu_vc = 
  read_csv("../data/Weekly_Cumulative_Influenza_Vaccination_Coverage_by_Flu_Season,_Selected_Demographics,_and_Race_and_Ethnicity_Among_Children_6_Months-17_Years,_United_States_20251003.csv") %>%
...
# Adult VC
adult_flu_vc = 
  read_csv("../data/Weekly_Influenza_Vaccination_Coverage_and_Intent_for_Vaccination,_Overall,_by_Selected_Demographics_and_Jurisdiction,_Among_Adults_18_Years_and_Older_20251003.csv") %>%
  dplyr::filter(Influenza_Season == "2024-2025") %>%
  dplyr::filter(`Geographic Level` == "State") %>%
  dplyr::filter(indicator_label == "Up-to-date") %>%
...
write.csv(
  weekly_ts,
  "../data/all_US_weekly_vax_distribution.csv",
  row.names = FALSE, quote = FALSE
)
